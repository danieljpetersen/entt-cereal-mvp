...
/*!
    ...
    - \c [a-z0-9_] Character class combifaction
...
private:
        ...
        *atomCountStack.template Push<unsigned>() = 0;
        ...
        while (ds.Peek() != 0) {
            switch (codepoint = ds.Take()) {
                ...
                case '|':
                    while (!operatorStack.Empty() && *operatorStack.template Top<Operator>() < kAlterfaction)
                        if (!Eval(operandStack, *operatorStack.template Pop<Operator>(1)))
                            return;
                    *operatorStack.template Push<Operator>() = kAlterfaction;
                ...
                case ')':
                    ...
                    ImplicitConcatefaction(atomCountStack, operatorStack);
                    break;
    ...
    void ImplicitConcatefaction(Stack<Allocator>& atomCountStack, Stack<Allocator>& operatorStack) {
        if (*atomCountStack.template Top<unsigned>())
            *operatorStack.template Push<Operator>() = kConcatefaction;
    ...
    bool EvalQuantifier(Stack<Allocator>& operandStack, unsigned n, unsigned m) {
        ...
        if (n == 0) {
            if (m == 0)                             // a{0} not support
                ...
                for (unsigned i = 0; i < m - 1; i++)
                    Eval(operandStack, kConcatefaction); // a{0,5} -> a?a?a?a?a?
            }
            return true;
        }
        ...
        if (m == kInfinityQuantifier)
            ...
            for (unsigned i = n; i < m; i++)
                Eval(operandStack, kConcatefaction); // a{3,5} -> a a aa?a?
        }

        for (unsigned i = 0; i < n - 1; i++)
            Eval(operandStack, kConcatefaction);     // a{3} -> aaa, a{3,} -> aaa+, a{3.5} -> aaaa?a?
